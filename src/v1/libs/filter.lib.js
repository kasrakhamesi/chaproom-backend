  ...
  try {
    ...
    for (const key in query) {
      if (
        ...
        String(key).toLowerCase() === 'withbalance'
      )
        continue
...
const searchStructre = async (model, value) => {
  try {
    ...
    if (
      model.tableName === 'treasruies' ||
      model.tableName === 'swap_providerscurrencynames' ||
      model.tableName === 'ex_providerscurrencynames'
    ) {
    ...
    if (
      model.tableName === 'swap_currency_pairs' ||
      model.tableName === 'ex_currency_pairs' ||
      model.tableName === 'ex_orders' ||
      model.tableName === 'swap_orders'
    ) {
      relationConditions.push(
        {
          '$to_currency.display_name$': {
            [Op.like]: `%${value}%`
          }
        },
        {
          '$from_currency.display_name$': {
            [Op.like]: `%${value}%`
          }
        },
        {
        ...
        {
          '$from_currency.protocol$': {
            [Op.like]: `%${value}%`
          }
        },
        {
          '$to_currency.protocol$': {
            [Op.like]: `%${value}%`
          }
        },
        {
          '$from_currency.subcategory$': {
            [Op.like]: `%${value}%`
          }
        },
        {
          '$to_currency.subcategory$': {
            [Op.like]: `%${value}%`
          }
        }
      )
    }
    if (model.tableName === 'admins') {
      relationConditions.push(
        {
          '$role.role_name$': {
            [Op.like]: `%${value}%`
        ...
        {
          '$role.permissions.perm_description$': {
            [Op.like]: `%${value}%`
          }
        }
      )
    ...
    if (model.tableName === 'api_keys') {
      relationConditions.push(
        ...
        {
          '$exchange_provider.provider_api_url$': {
            [Op.like]: `%${value}%`
          }
        }
      )
    ...
    if (model.tableName === 'activitylogs') {
      relationConditions.push(
        {
          '$admin.email$': {
            [Op.like]: `%${value}%`
        ...
        {
          '$admin.role.permissions.perm_description$': {
            [Op.like]: `%${value}%`
          }
        }
      )
    ...
    if (model.tableName === 'ex_orders') {
      relationConditions.push(
        {
          '$api_keys.provider_api_key$': {
            [Op.like]: `%${value}%`
          }
        },
        {
          '$api_keys.provider_api_secret$': {
            [Op.like]: `%${value}%`
          }
        },
        {
        ...
        {
          '$api_keys.role.exchange_provider.provider_name$': {
            [Op.like]: `%${value}%`
          }
        },
        {
          '$api_keys.role.exchange_provider.provider_api_url$': {
            [Op.like]: `%${value}%`
          }
        },
        {
    ...
    if (model.tableName === 'swap_orders') {
      relationConditions.push(
        {
          '$api_keys.provider_api_key$': {
            [Op.like]: `%${value}%`
        ...
        {
          '$swap_provider.provider_name$': {
            [Op.like]: `%${value}%`
          }
        },
        {
          '$swap_provider.provider_api_url$': {
            [Op.like]: `%${value}%`
          }
        }
      )
